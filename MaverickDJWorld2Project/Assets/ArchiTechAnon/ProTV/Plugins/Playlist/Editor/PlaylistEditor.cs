        ...
        private void parseContent(string text)
        {
            ...
            foreach (string l in lines)
            {
                ...
                if (line.StartsWith(newEntryIndicator))
                {
                    ...
                    if (autofillAltURL) alts[count] = new VRCUrl(autofillFormat.Replace("$URL", urls[count].Get()).Trim());
                    continue;
                }
                if (count == -1) continue;
        ...
        private void showListEntries()
        {
            ...
            for (var pageIndex = 0; pageIndex < currentPageUrls.Length; pageIndex++)
            {
                ...
                using (new EditorGUILayout.HorizontalScope())
                {
                    ...
                    var url = new VRCUrl(EditorGUILayout.TextField(oldUrl.Get(), GUILayout.ExpandWidth(true)).Trim());
                ...
                using (new EditorGUILayout.HorizontalScope())
                {
                    ...
                    var alt = new VRCUrl(EditorGUILayout.TextField(oldAlt.Get(), GUILayout.ExpandWidth(true)).Trim());
